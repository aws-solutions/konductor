...
//
// telemetry — usage-analytics telemetry (design doc:
// konductor-usage-analytics-design.md).
//
// `identity` owns `.konductor/telemetry-id.json` (D.1/D.2); `envelope`
// owns the per-invocation event schema and the real ingestion API's
// outer envelope (D.3/D.12); `report` owns the `telemetry::report_*`
// call-site API and transport (D.6/D.7). Re-exported flat from this
