...
pub fn dispatch_doctor_with(
    ...
    let mut results = run_checks(target_dir, from.as_deref(), &destination, home_dir_override);
    results.push(check_content_version(
        &destination,
        no_version_check,
        &latest_release_tag,
    ));
    ...
    if json {
        println!(
            "{}",
            format_report_json_with_cli_version(&cli_version_result, &results)
        );
    } else {
        print_report(std::slice::from_ref(&cli_version_result), verbose, color);
...
fn dispatch_doctor_all(
    ...
    for entry in &entries {
        ...
        let mut results = run_checks(target_dir, None, &destination, home_dir_override);
        results.push(check_content_version(
            &destination,
            no_version_check,
            &latest_release_tag,
    ...
    if json {
        println!(
            "{}",
            format_report_json_all_with_cli_version(&cli_version_result, &per_target)
        );
    } else {
        print_report(std::slice::from_ref(&cli_version_result), verbose, color);
...
fn format_report_json_all_with_cli_version(
    ...
    let mut doc: serde_json::Value = serde_json::from_str(&format_report_json_all(per_target))
        .expect("format_report_json_all always produces valid JSON");
    doc["cli_version"] = check_result_to_json(cli_version_result);
    if cli_version_result.status.is_failing() {
        doc["ok"] = serde_json::Value::Bool(false);
    }
    doc.to_string()
}

/// Single-target counterpart to `format_report_json_all_with_cli_version`:
/// same document `format_report_json` would produce for `results` alone,
/// plus `cli_version` folded in as its own top-level object -- NOT inside
/// `checks`, matching the `--all` path's shape exactly (see the doc
/// comment above and this feature's own AutoSDE-flagged inconsistency:
/// before this function existed, the single-target path folded
/// `cli_version` into `checks` while `--all` always lifted it to the top
/// level, so a `--json` consumer had to look in two different places for
/// the same machine-wide check depending on whether `--all` was passed).
fn format_report_json_with_cli_version(
    cli_version_result: &CheckResult,
    results: &[CheckResult],
) -> String {
    let mut doc: serde_json::Value = serde_json::from_str(&format_report_json(results))
        .expect("format_report_json always produces valid JSON");
    doc["cli_version"] = check_result_to_json(cli_version_result);
    if cli_version_result.status.is_failing() {
        doc["ok"] = serde_json::Value::Bool(false);
...
/// Renders a single `CheckResult` as the same JSON object shape
/// `format_report_json`'s own `checks` array entries use, without
/// wrapping it in a full report document -- the one place both
/// `format_report_json_all_with_cli_version` and
/// `format_report_json_with_cli_version` extract `cli_version`'s own
/// object to splice in at the top level, so the two can never drift on
/// what that object looks like.
fn check_result_to_json(result: &CheckResult) -> serde_json::Value {
    let doc: serde_json::Value =
        serde_json::from_str(&format_report_json(std::slice::from_ref(result)))
            .expect("format_report_json always produces valid JSON");
    doc["checks"][0].clone()
}
...
fn resolve_latest_release_tag(
    ...
    Some(super::install::github::fetch_latest_release_tag(
        "aws-solutions",
        "konductor",
        false,
    ))
    ...
    match latest_release_tag {
        Some(Ok(latest_tag)) => {
            ...
            if latest_version == running_version {
                CheckResult::ok(
                    "cli_version",
                    format!("running konductor {running_version}, which is current"),
                )
            } else {
                CheckResult::warn(
                    "cli_version",
                    format!(
                        "running konductor {running_version}, but {latest_version} is \
                         available"
                    ),
                    "run `konductor update --cli` to self-replace the binary on PATH",
                )
            }
        }
        Some(Err(err)) => CheckResult::info(
            "cli_version",
            format!("could not determine the latest available CLI version: {err}"),
...
fn check_content_version(
    ...
    let recorded = crate::cli::telemetry::read_install_info(destination)
        .and_then(|record| record.agent_version);
    let Some(recorded) = recorded else {
        return CheckResult::info(
            "content_version",
            format!(
                "no recorded content version for {} (never installed, or predates version \
                 tracking)",
                destination.display()
            ),
            "run `konductor install` or `konductor update` to record a content version",
        );
    };
    match latest_release_tag {
        Some(Ok(latest_tag)) => {
            let latest_version = latest_tag.trim_start_matches('v');
            if latest_version == recorded {
                CheckResult::ok(
                    "content_version",
                    format!(
                        "{} is at content version {recorded}, which is current",
                        destination.display()
                    ),
                )
            } else {
                CheckResult::warn(
                    "content_version",
                    format!(
                        "{} is at content version {recorded}, but {latest_version} is \
                         available",
                        destination.display()
                    ),
                    format!(
                        "run `konductor update --target {}` to refresh this target",
                        destination.display()
                    ),
                )
            }
        ...
        Some(Err(err)) => CheckResult::info(
            "content_version",
            format!("could not determine the latest available content version: {err}"),
            "check again later, or pass --no-version-check to skip this check",
        ),
        None => {
            // `no_version_check` already returned early above for this
...
mod tests {
    ...
    fn check_cli_version_with_no_version_check_skips_the_network_call() {
        ...
        assert_eq!(result.name, "cli_version");
        assert_eq!(result.status, CheckStatus::Info);
    ...
    fn check_content_version_with_no_version_check_skips_the_network_call() {
        ...
        let result = check_content_version(&destination, true, &None);
        assert_eq!(result.name, "content_version");
        assert_eq!(result.status, CheckStatus::Info);
        assert!(!result.status.is_failing());
        assert!(result.summary.contains("skipped"));
        fs::remove_dir_all(&destination).ok();
    ...
    fn check_cli_version_never_reports_failed_or_stale() {
        ...
        let result = check_cli_version(false, &latest_release_tag);
        assert_eq!(result.name, "cli_version");
        assert!(
            !result.status.is_failing(),
            "cli_version must never be Failed/Stale, got: {:?}",
            result.status
        );
    }
    ...
    fn check_content_version_never_reports_failed_or_stale() {
        let destination = scratch_dir("content-version-severity-dest");
        let latest_release_tag = resolve_latest_release_tag(false);
        let result = check_content_version(&destination, false, &latest_release_tag);
        assert_eq!(result.name, "content_version");
        assert!(
            !result.status.is_failing(),
            "content_version must never be Failed/Stale, got: {:?}",
            result.status
        );
        fs::remove_dir_all(&destination).ok();
    }
    ...
    fn check_content_version_reports_info_when_no_version_recorded() {
        ...
        let result = check_content_version(&destination, false, &None);
        assert_eq!(result.status, CheckStatus::Info);
        assert!(result.summary.contains("no recorded content version"));
        fs::remove_dir_all(&destination).ok();
    }
    ...
    fn cli_version_and_content_version_share_no_name_or_summary_wording() {
        ...
        let cli_result = check_cli_version(true, &None);
        let content_result = check_content_version(&destination, true, &None);
        assert_ne!(cli_result.name, content_result.name);
        assert_ne!(cli_result.summary, content_result.summary);
        fs::remove_dir_all(&destination).ok();
    }
    ...
    fn run_checks_includes_content_version_but_not_cli_version() {
        ...
        let destination = scratch_dir("run-checks-content-version-dest");
        let home = scratch_dir("run-checks-content-version-home");
        fs::create_dir_all(source.join("agents")).unwrap();
        ...
        assert!(
            ...
            "cli_version must never appear in run_checks -- it has no per-target dimension"
        );
    ...
    fn dispatch_doctor_with_includes_both_new_checks_under_no_version_check() {
        ...
        let destination = scratch_dir("dispatch-doctor-new-checks-dest");
        let home = scratch_dir("dispatch-doctor-new-checks-home");
        fs::create_dir_all(source.join("agents")).unwrap();
        fs::create_dir_all(source.join("skills")).unwrap();
        fs::create_dir_all(source.join("agent-sops")).unwrap();
        fs::create_dir_all(source.join("context")).unwrap();
        ...
        results.push(check_content_version(&destination, true, &None));

        let cli_version_count = results.iter().filter(|r| r.name == "cli_version").count();
        let content_version_count = results
            .iter()
            .filter(|r| r.name == "content_version")
            .count();
        assert_eq!(cli_version_count, 1, "cli_version must appear exactly once");
        assert_eq!(
            content_version_count, 1,
            "content_version must appear exactly once"
        );
        assert!(!results.iter().any(|r| r.status.is_failing()));
    ...
    fn format_report_json_with_cli_version_lifts_cli_version_to_the_top_level() {
        ...
        let content_version_result =
            check_content_version(&PathBuf::from("/tmp/does-not-matter"), true, &None);
        let results = vec![content_version_result];
        ...
        assert!(
            !checks.iter().any(|c| c["name"] == "cli_version"),
            "cli_version must not also appear inside checks -- it belongs at the top level only"
        );

        // content_version (and every other per-target/per-run check)
        // still appears inside `checks`, unaffected by this change.
        assert!(checks.iter().any(|c| c["name"] == "content_version"));
    }
    /// whole `--all` batch (never once per target), while
    /// `content_version` runs once PER tracked target. Two tracked
    /// targets under `--all --json --no-version-check` must show
    /// `cli_version` appearing at the top level once, and
    /// `content_version` appearing inside EACH target's own `checks`
    /// array.
    #[test]
    fn dispatch_doctor_all_runs_cli_version_once_and_content_version_per_target() {
        let _home = HomeGuard::new("doctor-all-new-checks-home");
        let cwd = scratch_dir("doctor-all-new-checks-cwd");
        let dest_a = scratch_dir("doctor-all-new-checks-dest-a");
        let dest_b = scratch_dir("doctor-all-new-checks-dest-b");
        ...
        for dest in [&dest_a, &dest_b] {
            ...
            manifest::upsert_strategy(dest, manifest).unwrap();
            let canonical = index::canonicalize_target_dir(dest).unwrap();
        ...
        let mut per_target: Vec<(String, Vec<CheckResult>)> = Vec::new();
        for dest in [&dest_a, &dest_b] {
            let mut results = run_checks(&cwd, None, dest, None);
            results.push(check_content_version(dest, true, &None));
            per_target.push((dest.display().to_string(), results));
        }
        let doc = format_report_json_all_with_cli_version(&cli_version_result, &per_target);
        let parsed: serde_json::Value = serde_json::from_str(&doc).unwrap();
        ...
        for target in targets {
            let checks = target["checks"].as_array().unwrap();
            let count = checks
                .iter()
                .filter(|c| c["name"] == "content_version")
                .count();
            assert_eq!(
                count, 1,
                "each target must carry exactly one content_version check"
            );
        }

        fs::remove_dir_all(&cwd).ok();
        fs::remove_dir_all(&dest_a).ok();
        fs::remove_dir_all(&dest_b).ok();
    }
